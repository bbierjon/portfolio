...
class Scene {
  ...
  async loadResources() {
    ...
    */
    ...
    // LOAD OFFICE
    const gltf = await this.gltfLoader.loadAsync('/models/Offices/officeTestF.glb');
    ...
    const tubeLightMaterial = new THREE.MeshBasicMaterial({ color: 0x0025FF });
    const deskLightMaterial = new THREE.MeshBasicMaterial({ color: 0x0E7DFF });
    const self = this;
    // SETUP CUSTOM MATERIALS
    const textureLoader = new THREE.TextureLoader()
    const bakedTexture = textureLoader.load('./models/textures/BakedTestF.jpg');
